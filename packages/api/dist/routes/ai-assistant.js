...
router.get("/jobs/:jobId/ai-optimize", (0, authorization_1.requirePermission)(Permissions_1.Permission.JOBS_READ), async (req, res) => {
    try {
        ...
        const userId = req.userId;
        // Get job details
        const jobs = await (0, db_1.query)(`SELECT id, rules, source_adapter, target_adapter FROM jobs WHERE id = $1 AND user_id = $2`, [jobId || null, userId]);
