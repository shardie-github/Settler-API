 ...
 */
exports.observabilityRouter = (0, express_1.Router)();
/**
 * GET /api/v1/observability/metrics
 * Get system and application metrics
 */
exports.observabilityRouter.get("/metrics", async (req, res) => {
    try {
        const userId = req.userId;
        const tenantId = req.tenantId;
        // Get job metrics
        const jobStats = await (0, db_1.query)(`SELECT 
        COUNT(*) as total,
      ...
      WHERE user_id = $1 AND tenant_id = $2`, [userId, tenantId]);
        ...
        const reconciliationStats = await (0, db_1.query)(`SELECT 
      ...
      AND created_at >= NOW() - INTERVAL '30 days'`, [userId, tenantId]);
        // Get API usage metrics
        const apiUsage = await (0, db_1.query)(`SELECT 
      ...
      FROM api_logs
      WHERE user_id = $1 AND tenant_id = $2
      AND created_at >= NOW() - INTERVAL '24 hours'`, [userId, tenantId]);
        // Get webhook metrics
        const webhookStats = await (0, db_1.query)(`SELECT 
 ...
 */
exports.observabilityRouter.get("/logs", async (req, res) => {
    try {
        const userId = req.userId;
        const tenantId = req.tenantId;
    ...
    `;
        ...
        const logs = await (0, db_1.query)(queryStr, params);
        res.json({
            data: logs,
            pagination: {
 ...
 */
exports.observabilityRouter.get("/traces", async (req, res) => {
    try {
 ...
 */
exports.observabilityRouter.get("/health", async (_req, res) => {
    try {
