...
exports.FeeExtractionService = void 0;
class FeeExtractionService {
    /**
     ...
     * Extract Stripe fees
     */
    extractStripeFees(transaction, tenantId) {
        const fees = [];
     ...
     * Extract PayPal fees
     */
    extractPayPalFees(transaction, tenantId) {
        const fees = [];
        const payload = transaction.rawPayload;
     ...
     * Extract Square fees
     */
    extractSquareFees(transaction, tenantId) {
        const fees = [];
        const payload = transaction.rawPayload;
        ...
        if (payload.processing_fee_money) {
            fees.push({
                ...
                rate: this.calculateRate(parseFloat(payload.processing_fee_money.amount || '0'), transaction.amount.value * 100),
     ...
     */
    extractGenericFees(transaction, tenantId) {
        const fees = [];
        const payload = transaction.rawPayload;
     ...
     */
    calculateFXFee(transaction, balanceTx) {
     ...
     */
    calculatePayPalFXFee(transaction, payload) {
        if (!payload.exchange_rate || payload.exchange_rate === 1) {
            return null;
        }
     ...
     */
    calculateRate(feeAmount, transactionAmount) {
     ...
     */
    calculateTotals(fees, transaction) {
...
exports.FeeExtractionService = FeeExtractionService;
//# sourceMappingURL=FeeExtractionService.js.map