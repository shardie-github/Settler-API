...
const types_1 = require("@settler/api/domain/canonical/types");
...
exports.PayPalEnhancedAdapter = PayPalEnhancedAdapter;
 > , tenantId;
string;
enhanced_base_1.NormalizedEvent[];
{
    const events = [];
 ...
 */
async;
pollTransactions(config, enhanced_base_1.AdapterConfig, dateRange, enhanced_base_1.DateRange);
Promise < types_1.Transaction[] > {
 ...
 */
async;
pollSettlements(config, enhanced_base_1.AdapterConfig, dateRange, enhanced_base_1.DateRange);
Promise < types_1.Settlement[] > {
    const: clientId = config.clientId,
    const: clientSecret = config.clientSecret,
    if(, clientId) { }
} || !clientSecret;
 ...
 */
async;
extractFees(transaction, types_1.Transaction, tenantId, string);
Promise < types_1.Fee[] > {
    const: fees, Fee: types_1.Fee, []:  = [],
 ...
 */
normalizeTransaction(raw, (Record), tenantId, string);
types_1.Transaction;
{
 ...
 */
normalizeSettlement(raw, (Record), tenantId, string);
types_1.Settlement;
{
 ...
 */
normalizeRefundDispute(raw, (Record), tenantId, string, type, types_1.RefundDisputeType);
types_1.RefundDispute;
{
    return {
        id: this.generateId(),
        tenantId,
 ...
 */
handleVersionChange(oldVersion, string, newVersion, string);
void {
    : .supportedVersions.includes(newVersion)
};
{
    console.warn(`PayPal API version ${newVersion} is not officially supported. Supported versions: ${this.supportedVersions.join(', ')}`);
}
mapTransactionType(payment, any);
types_1.TransactionType;
{
...
mapTransactionStatus(payment, any);
types_1.TransactionStatus;
{
    if (payment.state === 'refunded')
        return 'refunded';
    if (payment.state === 'denied' || payment.state === 'failed')
...
mapSettlementStatus(payout, any);
types_1.SettlementStatus;
{
...
mapRefundDisputeStatus(raw, any);
types_1.RefundDisputeStatus;
{
